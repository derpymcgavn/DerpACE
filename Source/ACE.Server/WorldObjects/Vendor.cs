...
namespace ACE.Server.WorldObjects
{
    ...
    public class Vendor : Creature
    {
        ...
        /// <summary>Unix timestamp (seconds) after which the random loot stock should be refreshed.</summary>
        private double _nextRestockTime = double.MaxValue;
        ...
        /// <summary>
        /// Schedules the next random-loot restock for a random interval between
        /// <see cref="DerpACEConfig.VendorRestockMinMinutes"/> and
        /// <see cref="DerpACEConfig.VendorRestockMaxMinutes"/>.
        /// </summary>
        private void ScheduleNextRestock()
        {
            var minSec = DerpACEConfig.VendorRestockMinMinutes * 60.0;
            var maxSec = DerpACEConfig.VendorRestockMaxMinutes * 60.0;
            var delay  = ThreadSafeRandom.Next((float)minSec, (float)maxSec);
            _nextRestockTime = Time.GetUnixTime() + delay;
        }

        /// <summary>
        /// Clears and re-rolls the random portion of the vendor's inventory.
        /// The base weenie items (CreateList / DestinationType.Shop) are preserved.
        /// </summary>
        private void RestockRandomInventory()
        {
            if (!DerpACEConfig.VendorRandomLootEnabled)
                return;

            // Remove only the items that were generated by LoadRandomLootInventory.
            var shopWcids = new System.Collections.Generic.HashSet<uint>(
                Biota.PropertiesCreateList
                     .Where(x => x.DestinationType == DestinationType.Shop)
                     .Select(x => x.WeenieClassId));

            var toRemove = DefaultItemsForSale
                .Where(kvp => !shopWcids.Contains(kvp.Value.WeenieClassId))
                .Select(kvp => kvp.Key)
                .ToList();

            foreach (var key in toRemove)
                DefaultItemsForSale.Remove(key);

            var uniqueGeneratedItems = UniqueItemsForSale
                .Where(kvp => IsVendorGeneratedUniqueItem(kvp.Value))
                .Select(kvp => kvp.Key)
                .ToList();

            foreach (var key in uniqueGeneratedItems)
                UniqueItemsForSale.Remove(key);

            // Re-roll fresh random loot
        ...
        public override void Heartbeat(double currentUnixTime)
        {
            ...
            if (DerpACEConfig.VendorRandomLootEnabled && currentUnixTime >= _nextRestockTime)
                RestockRandomInventory();
        }
        ...
        private void LoadInventory()
        {
            ...
            // Schedule the first restock cycle.
            ScheduleNextRestock();
        }
        ...
        /// rolls 1â€“10 (configurable) random items per category and adds them to the default inventory.
        ...
        private void LoadRandomLootInventory()
        {
            ...
            else
            {
                ...
                if (tier <= 0)
                    return; // not in a known town and no manual override â€” skip
            }
            ...
            if (profile == null)
            {
                ...
                profile = new Database.Models.World.TreasureDeath
                {
                    ...
                    MundaneItemTypeSelectionChances       = 7,   // profile7 = balanced pyreal/consumable/tool/component mundane loot
                };
            }
            ...
            foreach (var cat in categories)
            {
                ...
                for (var i = 0; i < count; i++)
                {
                    ...
                    AddVendorGeneratedUniqueItem(wo);
                }
            }
        }

        private void AddVendorGeneratedUniqueItem(WorldObject item)
        {
            item.ContainerId = Guid.Full;
            item.SoldTimestamp = null;
            item.VendorShopCreateListStackSize = 1;
            item.CalculateObjDesc();

            UniqueItemsForSale[item.Guid] = item;
        }

        private static bool IsVendorGeneratedUniqueItem(WorldObject item)
        {
            return item.SoldTimestamp == null && item.VendorShopCreateListStackSize == 1;
        }
        ...
        public bool BuyItems_ValidateTransaction(List<ItemProfile> itemProfiles, Player player)
        {
            ...
            if (AlternateCurrency == null)
            {
                // DerpACE: VendorsUseBank â€” count banked Pyreals toward purchase power
                var effectiveCoin = DerpAce.Bank.BankConfig.EnableBank && DerpAce.Bank.BankConfig.VendorsUseBank
                    ? (long)(player.CoinValue ?? 0) + player.GetCash()
                    : (long)(player.CoinValue ?? 0);

                if (effectiveCoin < totalPrice)
                {
                    CleanupCreatedItems(defaultItems);
                    return false;
                }
            ...
            else
            {
                ...
                // DerpACE: also count banked alt-currency items
                if (DerpAce.Bank.BankConfig.EnableBank && DerpAce.Bank.BankConfig.VendorsUseBank)
                {
                    var bankedItem = DerpAce.Bank.BankConfig.Items
                        .FirstOrDefault(i => i.Id == AlternateCurrency.Value);
                    if (bankedItem != null)
                        playerAltCurrency += (int)Math.Min(player.GetBanked(bankedItem.Prop), int.MaxValue);
                }
        ...
        private void RotUniques()
        {
            ...
            {
                if (IsVendorGeneratedUniqueItem(uniqueItem))
                    continue;
